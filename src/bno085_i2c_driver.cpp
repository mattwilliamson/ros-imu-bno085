...
bool BNO085I2CDriver::reset() {
    ...
    _i2c_smbus_write_byte_data(file, BNO085_OPR_MODE_ADDR, BNO085_OPERATION_MODE_CONFIG);
    ...
    // normal power mode
    _i2c_smbus_write_byte_data(file, BNO085_PWR_MODE_ADDR, BNO085_POWER_MODE_NORMAL);
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
    ...
    _i2c_smbus_write_byte_data(file, BNO085_OPR_MODE_ADDR, BNO085_OPERATION_MODE_NDOF);
    std::this_thread::sleep_for(std::chrono::milliseconds(25));
...
void BNO085I2CDriver::init() {
    ...
    std::cerr << "rev ids:"
      ...
      << " bl:" << _i2c_smbus_read_byte_data(file, BNO085_BL_REV_ID_ADDR) << std::endl;
...
IMURecord BNO085I2CDriver::read() {
    IMURecord record;
    unsigned char c = 0;
    ...
    if(_i2c_smbus_read_i2c_block_data(file, BNO085_ACCEL_DATA_X_LSB_ADDR, 0x20, (uint8_t*)&record) != 0x20) {
        throw std::runtime_error("read error");
    }
    if(_i2c_smbus_read_i2c_block_data(file, BNO085_ACCEL_DATA_X_LSB_ADDR + 0x20, 0x13, (uint8_t*)&record + 0x20) != 0x13) {
        throw std::runtime_error("read error");
    }
